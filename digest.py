...
def run_research():
    ...
    price_res = client.messages.create(
        ...
        messages=[{"role": "user", "content": "Today is " + today + ". Find current prices for: " + ticker_str + ", SPY, QQQ, VIX, GLD. Return ONLY JSON: {\"prices\": {\"AAPL\": {\"price\": 189.50, \"change_pct\": 1.2}}}"}]
    )
    price_json = {}
...
def build_email(report, price_data):
    ...
    for s in r.get("portfolio", []):
        ...
        ticker = s.get("ticker", "")
        sentiment = s.get("sentiment", "").upper()
        company = s.get("company", "")
        paragraph = s.get("paragraph", "")
        rows += '<tr><td style="padding:14px 16px;border-bottom:1px solid #E5E7EB"><b style="font-family:monospace">' + ticker + '</b> <span style="color:' + c + ';font-size:11px">' + sentiment + '</span> <span style="color:#6B7280">' + company + '</span><br><p style="font-size:13px;color:#374151;line-height:1.7">' + paragraph + '</p></td></tr>'
    ideas = ""
