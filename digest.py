...
def fetch_prices():
    """Pull live prices directly from Yahoo Finance via yfinance. No AI needed."""
    prices = {}
    tickers = [h["ticker"] for h in HOLDINGS]
    print("Fetching prices from Yahoo Finance: " + ", ".join(tickers))
...
def run_research(client, today, price_data):
    """Run AI research with confirmed prices pre-loaded."""
    ...
    prompt = (
        ...
        "For each holding write a 3-4 sentence research paragraph covering latest news, analyst views, and outlook. "
        "Provide exactly ONE high-conviction investment idea. "
        "Return ONLY valid JSON:\n"
        "{\"summary\":{\"outlook\":\"3-4 sentence macro outlook with specific data points\"},"
        "\"macro\":{\"fed_rate\":\"...\",\"ten_year\":\"...\",\"vix\":\"...\",\"cpi\":\"...\"},"
        "\"portfolio\":["
        "{\"ticker\":\"NVDA\",\"company\":\"NVIDIA Corp\",\"sentiment\":\"bullish\","
...
def build_email(report, price_data):
    ...
    # Portfolio summary using real Yahoo Finance prices
    total_cost = 0
    total_value = 0
    for h in HOLDINGS:
    ...
    for s in report.get("portfolio", []):
        ...
        # Always use Yahoo Finance price
        p = prices.get(ticker, {})
    ...
    claude_btn += "<a href='" + claude_url + "' style='display:inline-block;background:#2563EB;color:#fff;font-size:13px;font-weight:600;padding:12px 28px;border-radius:8px;text-decoration:none'>&#x1F4AC; Ask Claude about this brief &rarr;</a>"
...
if __name__ == "__main__":
    ...
    print("Step 2: Waiting 60 seconds before research call...")
    time.sleep(60)
